...
## What's wrong with the default, and what to use instead

Web fetch is the second most important tool an agent has, right after web
search. And the default one that ships with the Claude apps has two big
downsides.

- **It's held to ransom by `robots.txt`.** That mechanism was invented to
  instruct crawlers that sweep entire sites indiscriminately. I'd argue it's
  the wrong instrument for an agent fetching one specific page at the express
  request of a human. The page is public; the human could open it in a tab. The
  agent is just reading it on the human's behalf.
...
- it ignores `robots.txt`, as a browser driven by a human's intent should;
...
**Playwright MCP has no authentication of its own, and the AI apps only speak
OAuth.** The server's README says plainly that it "is not a security
boundary". Anyone who can reach the port owns a browser inside your network.
You'd think a bearer token in front would do, but the connector settings in
claude.ai, Claude Desktop and ChatGPT accept a URL and, optionally, OAuth. No
custom headers, no static tokens, no client certificates. So the missing piece
is an OAuth layer you don't have to write.
...
Claude Desktop, Claude Code and ChatGPT expect. On every call the portal adds a
service token to the upstream request, and an Access application on the
upstream hostname admits only that token. The result is that your server is
reachable through the portal and through nothing else. You also get per-tool
enable/disable and a request log for free.

**Cloudflare Tunnel, another free Cloudflare service, can keep the server off
...
exploit lands straight in the container's user context. The image already
runs as a non-root user; add a read-only root filesystem, all capabilities
dropped, no privilege escalation, the runtime's default seccomp profile, and
no Kubernetes service-account token mounted, because the server never talks
...
browser will happily open your cluster's API server at `http://10.43.0.1/`,
and it will follow redirects there too; Playwright's own `--blocked-origins`
flag is documented as not being a security boundary. This one bit me. My
first version was reachable through the cluster's ingress controller, which
also has an address on the LAN. Any phone, smart plug or guest on the Wi-Fi
could send a request with the right `Host` header, get an unauthenticated
browser session, and drive it wherever it liked. I proved it with one navigate
call to my Vault server's internal address, which cheerfully returned its
health JSON, cluster id and all. That's server-side request forgery from
anything on the Wi-Fi, and the carefully locked front door was irrelevant
because there was a second door.
...
- **Egress:** cluster DNS, then the whole internet with the private ranges
  ...
  `169.254.0.0/16` and `100.64.0.0/10`, plus their IPv6 equivalents if your
  cluster routes IPv6.
...
The same egress rule covers a nastier attacker. A web page can try to talk the
agent into pointing the browser at something on your LAN, and the agent may
well fall for it. It doesn't matter: the browser gets the same connection
refused. Prompt injection can still steer the browser around the public
internet, and it can still nudge whatever other tools your agent has. That's a
different post.

## Was it worth it?

Phoebe now reads Garmin release notes, and everything else I point her at,
including pages that need a login, a scroll, or JavaScript to produce their
content. The difference is not speed; a round trip through Cloudflare to a
Raspberry Pi and back takes a few seconds. The difference is that pages which
used to return nothing now return their content. Given that the whole thing
costs nothing beyond electricity for a Pi that was already plugged in, I'd say
the watch firmware was the most productive boring update I've ever installed.
